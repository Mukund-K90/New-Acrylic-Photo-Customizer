...
const menuItems = [
  ...
  { text: "Acrylic Wall Clock", icon: <IoMdClock size={30} color="#0056B3" />, path: "/acrylic-wall-clock" },
  { text: "Collage Acrylic Photo", icon: <RiCollageFill size={30} color="#0056B3" />, path: "/collage-acrylic-photo" },
];
    ...
    <Drawer anchor="left" open={open} onClose={onClose}>
      <Box sx={{ width: 280 }}>
        <Box sx={{ display: "flex", justifyContent: "flex-end", p: 1 }}>
          <IconButton onClick={onClose} sx={{ color: "#007BFF" }}>
            <CloseIcon />
          </IconButton>
        </Box>
        <List>
          {menuItems.map(({ text, icon, path }) => (
            <ListItemButton
              ...
              sx={{
                ...
                borderLeft: location.pathname === path ? "4px solid #007BFF" : "none",
              }}
            >
